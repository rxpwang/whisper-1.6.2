...
for i in $(seq 1 $repetitions); do
    ...
    log_file="benchmarking_results/whisper_streaming_cpp_optimized_serialized_${model}_${sample}_dtw-${dtw}_step-${step}_t-${threads}_run-${i}.log"
    log_file_no_audio_tag="benchmarking_results/whisper_streaming_cpp_optimized_serialized_no_audio_tag_${model}_${sample}_dtw-${dtw}_step-${step}_t-${threads}_run-${i}.log"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    system_profiler SPHardwareDataType > benchmarking_results/00-systeminfo_${model}_whisper_streaming_cpp_serialized.txt
fi
# Append the current git commit hash to the system info file
echo "Current Git Commit:" >> benchmarking_results/00-systeminfo_${model}_whisper_streaming_cpp_serialized.txt
git rev-parse HEAD >> benchmarking_results/00-systeminfo_${model}_whisper_streaming_cpp_serialized.txt