...
#include "whisper.h"
#include "ggml.h"
#include "examples/whisper_streaming_cpp_optimized/buffer.h"    // fxl: FIX THIS 
...
public:
    ...
    AudioStreamer(void (*callback_new_audio_chunk)(double, double, std::vector<float>&)): isStreaming(true), callbackNewAudioChunk(callback_new_audio_chunk) {}
...
private:
    ...
    void (*callbackNewAudioChunk)(double, double, std::vector<float>&); // Store the callback
    ...
    void captureAudio(const void *inputBuffer, unsigned long framesPerBuffer) {
        if (inputBuffer) {
            ...
            std::vector<float> chunk(input, input + framesPerBuffer);
            if (callbackNewAudioChunk) {
                callbackNewAudioChunk(0, 0, chunk);
            }
            std::lock_guard<std::mutex> lock(bufferMutex);
...
bool get_audio_chunk_from_mic(AudioStreamer &streamer, std::vector<float> &pcmf32_all, 
...
// fxl: print out a list of tokens 
...
// (fxl: trim the audio buffer  "segment" -- as output from Whisper. 
//  also trim the transcript buffer, seeb elow
...
// (fxl: a segment of transcript, with beg/end time for the segment 
...
/* fxl: callbacks needed: 
    ...
    - update of the current transcript buffer //any token(s) "uncofirmed" 
    - new audio chunk (with data
    - update of the current audio buffer
    (perf stats)
    - update of avg token lat
    (status)
    on/off of VAD
*/
    ...
    ) 
{
    ...
    AudioStreamer streamer(callback_new_audio_chunk);
    ...
    */
    ...
    // main audio loop          fxl: factor out as a thread
    while (is_running) {
        is_running = sdl_poll_events();  // fxl: wont need this
        if (!is_running) {
            break;
        }
        ...
        // run the inference
        {
            ...
            // whisper_streaming asr.transcribe() in an iter        fxl: "pcmf32" - audio buf
            ...
            std::vector<std::tuple<double, double, std::string>> tsw = output_word_level_timestamp(ctx, params, true); // fxl:inference res
            ...
            std::vector<std::tuple<double, double, std::string>> o = transcript_buffer.flush(); // fxl:local-2 here? "o" output tokens just confirmed
            if (callback_confirm_tokens)
                callback_confirm_tokens(o); // o: newly confirmed tokens
            committed.insert(committed.end(), o.begin(), o.end());  // fxl: "committed" -- all confirmed tokens since beg 
            ...
            std::string complete_transcript = std::get<2>(completed);   // fxl: accumulated but unflushed tokens (unused??
            std::string incomplete_transcript = std::get<2>(the_rest);  // fxl:unconfirmed tokens

            if (callback_update_unconfirmed) {
                callback_update_unconfirmed(r_o); // a snapshot of unconfirmed tokens
            }
            ...
            for (auto& token : o) { // fxl: each output token...
                double start_time, end_time;
                std::string transcript;
            ...
            // print result (fxl: relation with print above??
            {
                // fxl: print time info...
                if (!use_vad) {
                    #if 0 
                ...
                const int n_segments = whisper_full_n_segments(ctx); // fxl:print per "segment"
                for (int i = 0; i < n_segments; ++i) {
                    const char * text = whisper_full_get_segment_text(ctx, i);
                    ...
                    if (params.no_timestamps) {
                        ...
                        */
                        ...
                        //printf("%s", output.c_str());         // fxl: skipped printing
                        fflush(stdout);
                        /*
                        if (params.fname_out.length() > 0) {
                            fout << output;
    ...
    */
    ...
    while (is_running) {
        ...
        // run the inference
        {
            ...
            //printf("INCOMPLETE: %s\n", incomplete_transcript.c_str());
            if (callback_update_unconfirmed) {
                callback_update_unconfirmed(r_o);
            }
