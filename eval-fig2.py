...
if __name__ == '__main__':
    ...
    #step_len_list = [100, 250, 500, 750, 1000]
    step_len_list = [10]
    for step_len in step_len_list:
        print(f"Current step length: {step_len}")
        for path in tqdm(os.listdir(data_dir)):
            if path.endswith(".wav"):
                data_path = os.path.join(data_dir, path)
                result_path = os.path.join(result_dir, f"{os.path.splitext(os.path.basename(path))[0]}_step_len_{step_len}.log")
                ...
                elif args.system == "whisperflow_serialized":
                    result_path = os.path.join(result_dir, f"{os.path.splitext(os.path.basename(path))[0]}_step_len_{step_len}_serialized.log")
                    program = "./whisper_streaming_cpp_optimized"
                    arg_list = [
                        # ./whisper_streaming_cpp_optimized -m models/$model $data_dir/$sample -kc -dtw $dtw -ac -1 -at audio_tag/${model_type}_0.5s_avg.csv --step $step -ct $cpu_threads -gt $gpu_threads -mdr $mdr >> $log_file 2>&1
                        program,
